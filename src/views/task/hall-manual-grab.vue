<template>
    ...
    <div class="hall-manual-grab">
        <van-nav-bar title="人工抢单" left-arrow @click-left="goBack" />
    ...
    methods: {
        getUserNoviceInfo() {
            api.getUserNoviceInfo().then((res) => {
                ...
                if (res.success) {
                    let data = res.data
                    this.is_account = data.is_account
        ...
        grabEvent(item) {
            ...
            api.userGrab({ order_id: item.id }).then((res) => {
                if (res.success) {
                    ...
                    if (res.message == '请设置您的买家账号') {
                        this.$router.push({ name: 'grab-setnum' })
                    }
                }
            })
        },
