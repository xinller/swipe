            ...
            >
                ...
                <div class="com-shop-box" v-for="item in dataList" :key="item.id">
                    <div class="com-shop-top">
                        ...
                        <span class="c-primary">{{ item.status | format_status }}</span>
                    </div>
                    <div class="com-shop-card">
                        ...
                        <div class="content">
                            ...
                            <div class="right">
                                ...
                                <!-- 继续按钮 跳 货比打标 -->
                                <button @click="goNext(item)" v-if="item.status == 1">继续</button>
                                <button @click="toReceive(item)" v-if="item.step == 5">去收货</button>
                                <button @click="toGetMoney(item)" v-if="item.step == 7">确认收款</button>
                            </div>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        timeCount() {
            var token = this.$store.state.user.SWIPE_TOKEN
            if (token) {
        ...
        goDetail(item) {
            if (item.search_type == 0) {
                ...
                this.$router.push({ name: 'step-detail-zero', query: { id: item.id } })
            }
        },
        // 去收货
        toReceive(item) {
